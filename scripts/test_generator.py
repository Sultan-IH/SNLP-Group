...
ACTOR_CHECKPOINT = CHKPT_PATH / "generator_checkpoint76.pth.tar"  #  76th, on daily dialogue
...
def load_data_eval(path='dataset.pickle'):
    """
    Load data set
    """
    if not os.path.isfile(path):
        ...
        valid_dataset = corpus.get_test_dataset(min_reply_length=MIN_SEQ_LEN, \
                                                max_reply_length=MAX_SEQ_LEN)

        with open(path, 'wb') as handle:
            pickle.dump(valid_dataset, handle, protocol=pickle.HIGHEST_PROTOCOL)

        valid_data_loader = DPDataLoader(valid_dataset, batch_size=BATCH_SIZE)
...
if __name__ == '__main__':
    ...
    # Load data set
    valid_data_loader = load_data_eval()
    corpus = valid_data_loader.dataset.corpus
    SOS = valid_data_loader.dataset.corpus.token_to_id(DPCorpus.SOS)
    EOU = valid_data_loader.dataset.corpus.token_to_id(DPCorpus.EOU)
    PAD = valid_data_loader.dataset.corpus.token_to_id(DPCorpus.PAD)

    gen = Generator(SOS, EOU, VOCAB_SIZE, GEN_HIDDEN_DIM, GEN_EMBEDDING_DIM, \
                    MAX_SEQ_LEN).to(DEVICE)

    assert ACTOR_CHECKPOINT.exists()

    gen.load_state_dict(torch.load(str(ACTOR_CHECKPOINT), map_location=DEVICE)['state_dict'])
    gen.eval()
    gen.teacher_forcing_ratio = 0

    for iter, (context, reply) in enumerate(valid_data_loader):
        context = context.t().to(DEVICE)
        reply = reply.t().to(DEVICE)
        output = gen.forward(context, torch.zeros_like(reply)).argmax(2)

        output_txt = ' '.join(corpus.ids_to_tokens([int(i) for i in output[:, 0]]))
        reply_txt = ' '.join(corpus.ids_to_tokens([int(i) for i in reply[:, 0]]))
        context_txt = ' '.join(corpus.ids_to_tokens([int(i) for i in context[:, 0]]))

        print(f'\n ITER -- {iter}')
        print('[CONTEXT]: ' + context_txt)
        print('[GENERATED]: ' + output_txt)
        print('[REAL]: ' + reply_txt)
